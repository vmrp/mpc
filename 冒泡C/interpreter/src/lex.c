...
#define MAX_CHAR_VALUE 255      /* 一个“char”数据类型表示的最大值 */
...
struct ReservedWord
{
    const char *Word;
    enum LexToken Token;
    const char *SharedWord; /* 在共享的字符串空间存储的字 */
};


static const char ReservedWordsString[] = "#define\0#else\0#endif\0#if\0#ifdef\0#ifndef\0#include\0\
auto\0break\0case\0char\0continue\0default\0delete\0do\0else\0enum\0extern\0for\0goto\0if\0int\0long\0\
new\0register\0return\0short\0signed\0sizeof\0static\0struct\0switch\0typedef\0union\0unsigned\0void\0while\0";

const enum LexToken TokenAll[] = {TokenHashDefine, TokenHashElse, TokenHashEndif, TokenHashIf, TokenHashIfdef, TokenHashIfndef, TokenHashInclude,
TokenAutoType, TokenBreak, TokenCase, TokenCharType, TokenContinue, TokenDefault, TokenDelete, TokenDo, TokenElse, TokenEnumType, TokenExternType,
TokenFor, TokenGoto, TokenIf, TokenIntType, TokenLongType, TokenNew, TokenRegisterType, TokenReturn, TokenShortType, TokenSignedType,
TokenSizeof, TokenStaticType, TokenStructType, TokenSwitch, TokenTypedef, TokenUnionType, TokenUnsignedType, TokenVoidType, TokenWhile};

static struct ReservedWord ReservedWords[37];
...
{
    int Count;
    const char *wStr = ReservedWordsString;

    LexValue.Val = &LexAnyValue;

    for (Count = 0; Count < sizeof(ReservedWords) / sizeof(struct ReservedWord); Count++)
    {
        ReservedWords[Count].Token = TokenAll[Count];
        ReservedWords[Count].Word = wStr;
        wStr += strlen(wStr) + 1; //移动指针到下个字符串
    ...
    do
    {
        *Value = &LexValue;
        ...
        switch (ThisChar)
        {
            ...
            default:  LexFail(Lexer, "非法字符'%d'", ThisChar); break;
        }
...
void LexHashIf(struct ParseState *Parser)
{
    ...
    if (Token == TokenIdentifier)
    {
        ...
        if (!TableGet(&GlobalTable, IdentValue->Val->Identifier, &SavedValue, NULL, NULL, NULL))
            ProgramFail(Parser, "'%s'未知", IdentValue->Val->Identifier);
        
        if (SavedValue->Typ->Base != TypeMacro)
            ProgramFail(Parser, "值无效");
